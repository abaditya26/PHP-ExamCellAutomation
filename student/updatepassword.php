...
if(isset($_POST['oldpassword'])){
    ...
    $query="SELECT `pass` FROM `studentuser` WHERE `uname` = '$uname'";
    $result=mysqli_query($conn,$query);
    if($result){
        $row=mysqli_fetch_row($result);
        if($row[0]==$oldpassword){
                $query1="UPDATE `studentuser` SET `pass`='$newpassword' WHERE `uname` = '$uname'";
...
<div class="container" style="max-width: 550px;">
    ...
    <form name="changepass" method="POST" action="#" onsubmit="return checkpass();" class="form" style="padding-top: 20px;">
